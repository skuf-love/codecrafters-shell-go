...
func historyExecutable(args []string, stdin []byte) []byte{
	...
	if argLen > 0 && args[0] == "-r" {
		...
		err := my_shell_history.ImportFromFile(pathArg)
		if err != nil {
	...
	if argLen > 0 && args[0] == "-w" {
		...
		err := my_shell_history.ExportToFile(pathArg, false)
		if err != nil {
			fmt.Printf("history: %v\n", err)
		}
		return make([]byte, 0)
	...
	if argLen > 0 && args[0] == "-a" {
		...
		err := my_shell_history.ExportToFile(pathArg, true)
		if err != nil {
			fmt.Printf("history: %v\n", err)
		}
		return make([]byte, 0)
